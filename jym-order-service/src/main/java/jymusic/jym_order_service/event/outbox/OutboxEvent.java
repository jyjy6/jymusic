...
public class OutboxEvent {
    ...
    /** EventEnvelope.eventId 와 동일한 UUID — 컨슈머 측 멱등성 키. */
    @Column(name = "event_id", nullable = false, length = 36)
    private String eventId;

    /** 집계 루트 종류 (예: ORDER, PAYMENT, STOCK). 운영 시 추적/필터링 용도. */
    @Column(name = "aggregate_type", nullable = false, length = 30)
    private String aggregateType;

    /** Kafka partition key 로도 사용됩니다 (예: orderId 문자열). */
    ...
    /** 내부 페이로드만 JSON 직렬화한 문자열. 발행 시 EventEnvelope 로 재포장됩니다. */
    @Lob
     ...
     * 발행 실패 시 호출.
     *
     * <p>retryCount 를 증가시키고 Exponential Backoff 으로 nextRetryAt 을 산정합니다.
     * retryCount 가 {@link #MAX_RETRY_BEFORE_FAILED} 에 도달하면 FAILED 로 격리되며
     * 더 이상 폴링되지 않습니다. 그 외엔 PENDING 으로 유지되어 nextRetryAt 도래 후
     * 다음 폴링 사이클에서 재시도됩니다.</p>
     *
     * <p>딜레이 공식: {@code min(BASE * 2^(retryCount - 1), MAX)} (단위 초).
     * 자세한 표는 SPEC §4.6 참조.</p>
     *
     * <p>MAX_ERROR_LENGTH 는 기본값 VARCHAR(255) 일 경우 긴 에러 저장 시 INSERT 가 실패하므로
     * VARCHAR(1000) 으로 강제하고, 그보다 긴 메시지는 잘라서 저장합니다.</p>
     */
